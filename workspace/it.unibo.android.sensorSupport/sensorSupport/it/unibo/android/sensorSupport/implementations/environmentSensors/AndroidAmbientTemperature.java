...
public class AndroidAmbientTemperature extends AndroidSensor implements IAndroidTemperature {
	
	private static AndroidAmbientTemperature[] instances = new AndroidAmbientTemperature[4];
	 ...
	 */
	public static AndroidAmbientTemperature getInstance(SensorManager manager, int delay){
		Sensor proximity = manager.getDefaultSensor(Sensor.TYPE_AMBIENT_TEMPERATURE);
		if(proximity != null){
			int index;
			switch (delay){
			...
			if (instances[index] == null)
				instances[index] = new AndroidAmbientTemperature(manager, proximity, delay);
	...
	private AndroidAmbientTemperature(SensorManager manager, Sensor proximity, int delay){
